...
function renderContainerSectionPanel(container, section, className = 'container-compartment-panel') {
  return `<section class="${className}" aria-label="Inventaire de ${escapeHtml(section.label)}">
    <div class="inventory-line-list">${section.items.map((item) => `<div class="inventory-line"><span class="inventory-line-leading">${item.pictogram ? `<span class="inventory-pictogram" role="img" aria-label="${escapeHtml(item.presentation || 'Présentation du produit')}">${escapeHtml(item.pictogram)}</span>` : ''}<span class="inventory-quantity">${Number(item.expectedQuantity)}×</span></span><span><strong>${escapeHtml(item.label)}</strong>${itemHasSourceAmbiguity(item) ? `<span class="data-quality-badge">${icon('alert', 12)} Libellé source à valider</span>` : ''}<small>${item.presentation ? `${escapeHtml(item.presentation)} · ` : ''}${escapeHtml(item.category)} · unité : ${escapeHtml(item.unit)}${item.packSize ? ` · ${item.packSize} par paquet` : ''}${item.expiryTracked ? ' · péremption suivie' : ' · réutilisable'}</small>${(item.validationIssues || []).map((issue) => `<small class="inventory-validation-issue">${escapeHtml(issue)}</small>`).join('')}${item.sourceText ? `<small class="inventory-source-text">Source : ${escapeHtml(item.sourceText)}</small>` : ''}</span></div>`).join('')}</div>
