...
function cartFront(cart, notice) {
  return `<section class="ecm-stage">
    <p class="ecm-instruction">Touchez une façade de tiroir pour afficher son contenu.</p>
    <div class="ecm-cart-canvas">
      <img src="${escapeHtml(cart.frontAsset)}" alt="${escapeHtml(cart.label)}">
      ${cart.drawers.map((drawer) => `<button type="button" class="ecm-drawer-hitbox ${drawer.available ? 'is-available' : 'is-pending'}" data-ecm-action="open-drawer" data-ecm-drawer="${escapeHtml(drawer.id)}" style="left:${drawer.hitArea.x}%;top:${drawer.hitArea.y}%;width:${drawer.hitArea.width}%;height:${drawer.hitArea.height}%" aria-label="${escapeHtml(drawer.available ? `Ouvrir ${drawer.label}` : `${drawer.label}, contenu non encore documenté`)}"><span>${escapeHtml(drawer.label)}</span></button>`).join('')}
    </div>
    <div class="ecm-drawer-list" aria-label="Accès direct aux tiroirs">
      ${cart.drawers.map((drawer) => `<button type="button" class="${drawer.available ? 'is-available' : 'is-pending'}" data-ecm-action="open-drawer" data-ecm-drawer="${escapeHtml(drawer.id)}">
...
function drawerView(drawer, selectedItem) {
  ...
  return `<section class="ecm-stage">
    ...
    <div class="ecm-drawer-canvas ${selectedItem ? 'has-selection' : ''}">
      <img class="ecm-drawer-background" src="${escapeHtml(drawer.topAsset)}" alt="">
